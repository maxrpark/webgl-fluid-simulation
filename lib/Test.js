...
var FluidSimulation = /** @class */ (function () {
  function FluidSimulation(canv) {
    ...
    var splat = function (x, y, dx, dy, color) {
      ...
      _this.webGLContext.gl.uniform1i(
        _this.splatProgram.uniforms.uTarget,
        _this.velocity.read.attach(0)
      );
      _this.webGLContext.gl.uniform1f(
        _this.splatProgram.uniforms.aspectRatio,
        _this.canvas.width / _this.canvas.height
      );
      _this.webGLContext.gl.uniform2f(_this.splatProgram.uniforms.point, x, y);
      _this.webGLContext.gl.uniform3f(
        _this.splatProgram.uniforms.color,
        dx,
        dy,
        0.0
      );
      _this.webGLContext.gl.uniform1f(
        _this.splatProgram.uniforms.radius,
        _this.correctRadius(config.SPLAT_RADIUS / 100.0)
      );
      _this.blit(_this.velocity.write);
      _this.velocity.swap();
      _this.webGLContext.gl.uniform1i(
        _this.splatProgram.uniforms.uTarget,
        _this.dye.read.attach(0)
      );
      _this.webGLContext.gl.uniform3f(
        _this.splatProgram.uniforms.color,
        color.r,
    ...
    this.canvas.addEventListener("mousedown", function (e) {
      ...
      var pointer = _this.pointers.find(function (p) {
        return p.id == -1;
      });
    ...
    window.addEventListener("keydown", function (e) {
      ...
      if (e.key === " ") _this.splatStack.push(Math.random() * 20 + 5);
    });
  ...
  FluidSimulation.prototype.blit = function (target) {
    ...
    var func = function (target, clear) {
      ...
      if (target == null) {
        _this.webGLContext.gl.viewport(
          0,
          0,
          _this.webGLContext.gl.drawingBufferWidth,
          _this.webGLContext.gl.drawingBufferHeight
        );
        _this.webGLContext.gl.bindFramebuffer(
          _this.webGLContext.gl.FRAMEBUFFER,
          null
        );
      } else {
      ...
      _this.webGLContext.gl.drawElements(
        _this.webGLContext.gl.TRIANGLES,
        6,
        _this.webGLContext.gl.UNSIGNED_SHORT,
        0
      );
    };
    func(target);
  ...
  FluidSimulation.prototype.createDoubleFBO = function (
    w,
    h,
    internalFormat,
    format,
  ...
  FluidSimulation.prototype.resizeFBO = function (
    target,
    w,
    h,
    internalFormat,
  ...
  FluidSimulation.prototype.resizeDoubleFBO = function (
    target,
    w,
    h,
    internalFormat,
  ...
  FluidSimulation.prototype.createFBO = function (
    w,
    h,
    internalFormat,
    format,
  ...
  FluidSimulation.prototype.applySunrays = function (
    source,
    mask,
    destination
  ) {
